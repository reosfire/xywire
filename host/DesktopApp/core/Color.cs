namespace Leds.core
{
    public readonly struct Color
    {
        ...
        {
            return RGB(a.Red + (b.Red - a.Red) * fraction, a.Green + (b.Green - a.Green) * fraction, a.Blue + (b.Blue - a.Blue) * fraction);
        }
        ...
        public static Color operator *(Color a, double factor)
        {
            return RGB(a.Red * factor, a.Green * factor, a.Blue * factor);
        }

        private static void EnsureValidRGBComponent(int value, string name)
        {
            if (value < 0 || value > 255) throw new ArgumentOutOfRangeException(name, $"Was: {value}, but should be in [0, 255]");
        }
        ...
        {
            if (hue < 0 || hue > 360) throw new ArgumentOutOfRangeException(nameof(hue), $"Was: {hue}, but should be in [0, 360]");
        }
        private static void EnsureValidFraction(double fraction, string name)
        {
            if (fraction < 0 || fraction > 1) throw new ArgumentOutOfRangeException(name, $"Was: {fraction}, but should be in [0, 1]");
        }
    }
}
