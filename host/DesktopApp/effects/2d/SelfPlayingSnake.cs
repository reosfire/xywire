...
namespace Leds.effects._2d
{
    internal class SelfPlayingSnake : AbstractEffect
    {
        ...
        protected override void MoveNext()
        {
            ...
            {
                Console.WriteLine("Snake ended up circling on seed: " + _currentSeed + "; empty cells count: " + (LedLine.Width * LedLine.Height - _snake.Count));
                RestoreInitialState();
                return;
            }
            ...
            var direction = GetNextDirection(currentGrid);
            
            var head = _snake.GetFront();
            var newHead = head + direction;
            
            if (IsIndexOutsideBounds(newHead))
            {
                Console.WriteLine("Snake ended up hitting wall on seed: " + _currentSeed + "; empty cells count: " + (LedLine.Width * LedLine.Height - _snake.Count));
                RestoreInitialState();
                return;
            }
            
            var cellAtNext = currentGrid.Get(newHead);
            if (cellAtNext is CellType.SnakeBody)
            {
                Console.WriteLine("Snake ended up biting itself on seed: " + _currentSeed + "; empty cells count: " + (LedLine.Width * LedLine.Height - _snake.Count));
                RestoreInitialState();
                return;
            }
            ...
            {
                _colorsBuffer.Set(_snake[i], Color.HSV(hue: i * 2 % 360, saturation: 1, value: 1));
            }
        ...
        private void SpawnFood(CellType[][] grid)
        {
            ...
            for (var i = 0; i < LedLine.Height; i++)
            {
                for (var j = 0; j < LedLine.Width; j++)
                {
        ...
        private Direction GetNextDirection(CellType[][] grid)
        {
            ...
            var head = _snake.GetFront();
            var tail = _snake.GetBack();
            
            var shortestToFood = _pathFinder.GetShortestPath(head, _foodIndex, grid);
            if (shortestToFood != null)
            {
                // If shortest path to food is safe, take it
                if (shortestToFood.Count > 1 && PathSafe(shortestToFood))
                {
                    var direction = GetDirectionFromPath(shortestToFood);
                    _cachedPathToApple = [];
                    _cachedPathToTail = [];
                    return direction;
                }
                
                var shortestSafeAppleEndingPath = _cachedPathToApple;
                for (var i = 0; i < AttemptsToFindShortestSafePathToApple; i++)
                {
                    GetRandomPath(head, grid, _randomPathBuffer);
                    if (_randomPathBuffer.Count == 0) continue;
                    if (_randomPathBuffer.Count >= shortestSafeAppleEndingPath.Count && shortestSafeAppleEndingPath.Count > 1) continue;
                ...
                if (shortestSafeAppleEndingPath.Count > 1)
                {
                    ...
                    var direction = GetDirectionFromPath(shortestSafeAppleEndingPath);
                    _cachedPathToTail = [];
                    return direction;
                }
            }
            ...
            for (var i = 0; i < AttemptsToFindLongestPathToTail; i++)
            {
                GetRandomPath(head, grid, _randomPathBuffer);
                if (!PathSafe(_randomPathBuffer)) continue;
            ...
            {
                var direction = GetDirectionFromPath(_cachedPathToTail);
                _cachedPathToApple = [];
                return direction;
            }

            // As final fallback, use the shortest path to the tail
            var shortestToTail = _pathFinder.GetShortestPath(head, tail, grid);
            if (shortestToTail != null && shortestToTail.Count >= 2 && PathSafe(shortestToTail))
            {
                var direction = GetDirectionFromPath(shortestToTail);
                _cachedPathToApple = [];
                _cachedPathToTail = [];
                return direction;
            }
            ...
            foreach (var direction in Direction.PossibleDirections)
            {
        ...
        private bool PathSafe(List<Index2D> path)
        {
            ...
            var simHead = _simSnake.GetFront();
            var simTail = _simSnake.GetBack();
            
            _simGrid.Set(simHead, CellType.SnakeHead);
            for (var i = 1; i < _simSnake.Count - 1; i++)
            {
        ...
        private void GetRandomPath(Index2D from, CellType[][] grid, List<Index2D> path)
        {
            ...
            var inPath = new BitArray(LedLine.Height * LedLine.Width)
            {
                [from.Row * LedLine.Width + from.Col] = true
            };

            while (true)
            {
                var current = path.Last();
                var possibleDirectionsMask = 0;
                var possibleDirectionsCount = 0;
                
                for (var i = 0; i < Direction.PossibleDirections.Length; i++)
                {
                    var direction = Direction.PossibleDirections[i];
                ...
                var randomDirectionIndex = _random.Next(possibleDirectionsCount);
                
                var takenDirectionIndex = GetIthSetBitIndex(possibleDirectionsMask, randomDirectionIndex);

                var moveTaken = current + Direction.PossibleDirections[takenDirectionIndex];
                path.Add(moveTaken);
        ...
        {
            var grid = Array2D.CreateJagged(LedLine.Height, LedLine.Width, CellType.Empty);
        ...
        private void RestoreInitialSnakeState()
        {
            ...
            var startRow = LedLine.Width / 2;
            var startCol = LedLine.Height / 2;
    ...
    internal static class Extensions
    {
        ...
        {
            return array[index.Row][index.Col];
        }
    ...
    internal class PathFinder(int width, int height)
    {
        ...
        public List<Index2D>? GetShortestPath(Index2D from, Index2D to, CellType[][] grid)
        {
            ...
            {
                var currentPos = _shortestPathQueue.Dequeue();

                if (currentPos == to) break;

                foreach (var direction in Direction.PossibleDirections)
                {
                    var next = currentPos + direction;
        ...
        public bool PathExists(Index2D from, Index2D to, CellType[][] grid)
        {
            ...
            {
                var currentPos = _pathExistsQueue.Dequeue();
    ...
    internal class CircularBuffer<T>(int capacity)
    {
        ...
        public bool AddBack(T item)
        {
            ...
            var end = (_start + Count) % _buffer.Length;
            _buffer[end] = item;
            Count++;
            return true;
        }
        ...
        public T RemoveBack()
        {
            ...
            var endIndex = (_start + Count - 1) % _buffer.Length;
            var item = _buffer[endIndex];
            Count--;
            return item;
        }
